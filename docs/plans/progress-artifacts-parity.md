...
**Queued for Codex (after PR 2 lands):** land the Grok **0.2.114** contract row in `runtime_tuning_profile`. The evidence is already staged on `work/claude-agents-drift`: `tests/fixtures/ai/grok/0.2.114/` (basic capture + manifest; grammar identical to 0.2.111, additive cost keys on the end envelope only, text envelopes still child-identity-free — so the subagents-off rule carries over). Re-capture parent-child at landing time per the fixture norm. Also update `agents_panel::TESTED_VERSIONS` (display mirror, cross-checked by test) in the same PR.

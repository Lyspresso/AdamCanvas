...
fn dotsField(uv01: vec2<f32>) -> vec4<f32> {
  ...
  // Apply brightness to coverage rather than the tint itself. This keeps the
  // white-on-black and black-on-white palettes as exact visual inverses.
  let amount = clamp(intensity * u.brightness, 0.0, 1.0);
  let col = mix(bg, u.tint.rgb, vec3<f32>(amount));
  return vec4<f32>(col, 1.0);
}
